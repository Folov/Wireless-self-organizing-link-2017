...
echo "find router"
while [ 1 ]; do
	if iw dev wlan10 scan >| /tmp/wsol/iwscan.txt
	then
		egrep -w '^	SSID|^	signal|^BSS' /tmp/wsol/iwscan.txt | sed 's/(on wlan10)//; s/	//; s/BSS //; s/signal: //; s/SSID: //; s/ dBm//; s/ -- associated//; s/^-//' >| /tmp/wsol/routerlist.txt
